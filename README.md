...
- **전체**: 업타임, 로그, 인프라, API 추적, 메트릭을 한 번에 설정합니다.
- **기본**: Docker 서비스 상태와 로그만 수집합니다.
- **사용자 지정**: 필요한 기능만 골라 권한과 수집기를 최소화합니다.

선택을 마치면 10분 동안 한 번만 사용할 수 있는 설치 명령이 표시됩니다. 장기 API 키는
브라우저에 노출되지 않고 설치 과정에서 대상 서버로 직접 전달됩니다.

### 3. 모니터링할 서버에 모니터링 번들 설치

설치기는 선택한 수집 범위에 필요한 구성만 실행합니다. **전체** 프로필이나 API 추적을
포함한 사용자 지정 프로필에서는 Docker 수집기와 권한이 분리된 OBI eBPF 관측기를 함께
실행합니다. **기본** 프로필은 eBPF 관측기 없이 Docker 서비스 상태와 로그만 수집합니다.
...
그 서버의 컨테이너도 자동으로 나타납니다. API 추적을 선택했다면 eBPF 관측기가 컨테이너
...
직접 OpenTelemetry로 연결한 서비스는 기존 ID와 수집 키를 유지한 채 필요한 신호 권한만
...
웹 UI에서 Docker 환경을 열고 **상세 API 모니터링**을 실행한 뒤 표시된 명령 한 줄을

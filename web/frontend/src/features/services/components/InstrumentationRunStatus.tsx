...
const reasons: Record<string, string> = { preflight_failed: 'Compose 경로·프로젝트·실행 상태를 확인하세요.', restart_failed: '서비스 재시작에 실패했습니다.', verification_failed: '컨테이너 상태·계측 옵션·볼륨·네트워크 검사에 실패했습니다.', manual_rollback: '수동 복구 명령이 실행됐습니다.' };
...
export function InstrumentationRunStatus({ agentId, runId }: { agentId: string; runId?: string }) {
  ...
  return <section aria-label="계측 실행 결과" className="space-y-2 rounded-xl border border-ui-border bg-ui-hover-soft p-4">
    <h4 className="type-label text-text-base">계측 실행 결과</h4>
    {error ? <p role="alert" className="type-body text-status-warn">{error}</p> : run && <>
      ...
      <p className="type-body text-text-secondary">{statuses[run.status]} · {new Date(run.updatedAt).toLocaleString()}</p>
      {run.reason && <p className="type-body text-status-warn">{reasons[run.reason] ?? run.reason}</p>}
      ...
      <p className="type-body text-text-muted">CLI 검증은 실행 상태·계측 설정을 확인합니다. 새 트레이스에는 기존 eBPF 수집도 포함될 수 있습니다. 트래픽이 없어도 자동 복구하지 않습니다.</p>
    </>}
  </section>;
}

...
import {
  ...
  niceYAxis, timeXAxisProps, useChartTheme, yAxisProps,
...
/** Reports the histogram's hovered/keyboard-focused bucket, so a click or Enter can pick it. */
...
function ServiceLogsPanel(props: Props) {
  ...
  const hoveredBucket = useRef<number | undefined>(undefined);
  const syncHoveredBucket = useCallback((t: number | undefined) => { hoveredBucket.current = t; }, []);
    ...
    <div className="space-y-4">
      ...
      {histogram.buckets.length > 0 && (() => {
        ...
        const data = fillBuckets(
          histogram.buckets.map((b) => ({ ...b, t: Date.parse(b.time) })),
          histogram.window,
          bucketMs,
        ...
        const pick = () => {
          if (hoveredBucket.current === undefined) return;
          setPicked({ from: hoveredBucket.current, range });
          setPage(1);
        };
        return (
            ...
            <div onKeyDown={(e) => { if (e.key === 'Enter') pick(); }}>
              <ResponsiveContainer width="100%" height={110} initialDimension={CHART_INITIAL_DIMENSION}>
                <BarChart data={data} margin={{ top: 4, right: 8, left: 0, bottom: 0 }} onClick={pick} style={{ cursor: 'pointer' }}>
                  ...
                  <XAxis {...timeXAxisProps(theme, histogram.window)} padding={{ left: 6, right: 6 }} />
                  <YAxis {...yAxisProps(theme, 36)} {...niceYAxis(peak)} allowDecimals={false} />
                  <Tooltip content={({ active, label, payload }) => (
                    <ChartTooltip active={active} label={label} payload={(payload as import('../../../components/charts').TooltipPayloadItem[] | undefined)?.filter((item) => Number(item.value) > 0)} unit="" theme={theme} valueFormatter={(v) => String(v)} />
                  )} />
                  <ActiveBucketSync onChange={syncHoveredBucket} />
                  {LEVEL_BAR.map((l) => (

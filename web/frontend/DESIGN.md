...
4. **캡션 줄** — 타일 카드에서만: 단위 · 범례 칩. 메트릭 탐색은 histogram 단독 보기에서 분포 줄(아래 "메트릭 탐색")
...
카드 여백은 `p-6` 하나. 차트 아래 표·설명 문장은 두지 않는다 — 메트릭 탐색 단독 보기의 시리즈 표(시리즈 2개 이상일 때)와 데이터가 잘렸다는 경고만 예외. 조작 안내("막대를 누르면…")는 `ChartTooltip`의 `footer`로 포인터가 있는 자리에 둔다.
...
| `splitGaps` `fillBuckets` | 공백·빈 버킷 |
| `rangeAreas` `thresholdLines` `areaGradient` `OPERATOR_LABEL` | 공백 음영(라벨 없음), 알림 임계값 점선, 채움 그라데이션, 임계값 연산자 기호 |
...
- **실패한 체크는 응답 시간이 아니다.** 타임아웃까지 기다린 시간이라 막대 높이·통계에서 뺀다 — 업타임 응답 시간은 아래 "업타임 응답 시간"의 체크별 막대로 그린다. 백엔드 버킷 평균도 성공한 체크만 집계한다
...
- **시리즈 2개 이상** → 제목 줄 오른쪽 범례 칩(`ChartLegend`). 시리즈별 통계 표(`ChartStatsLegend`)는 메트릭 탐색 단독 보기에서 시리즈가 2개 이상일 때만 — 칩 대신 차트 아래 왼쪽에 붙는 좁은 표로. 격자 패널은 칩이다. 카드 폭 전체로 펼친 표는 숫자가 시리즈 이름에서 멀어져 읽히지 않았다
...
**메트릭 탐색 (`AgentServiceMetricsTab`, 2026-09-26)** — 한 차트를 보려고 눈이 여섯 곳을 오간다는 피드백(*"정보가 너무 흩어져 있다"*)으로 고쳤다.
- **모든 메트릭을 한 번에 격자로 그린다** (Grafana 대시보드처럼 `lg` 이상 2열, 인프라 추세와 같은 틀). 메트릭을 하나씩 고르는 방식은 두 번 바꿨다 — 차트 아래 표는 고르면 화면 밖 차트가 바뀌었고, 왼쪽 레일은 *"한눈에 들어오지 않는다"*는 피드백을 받았다. 패널은 크로스헤어를 시간값으로 공유하고(`syncId` + `syncMethod="value"` — 메트릭마다 행 배열이 달라 index 동기화가 어긋난다) 툴팁은 마우스가 있는 패널에만. 순서는 **이름순** — API는 마지막 수신순이라 새로고침마다 격자가 뒤섞였다. 8개를 넘으면 위에 검색창
- **패널 제목을 누르면 그 메트릭만 크게 본다** (`?metric=`, Grafana의 View panel). 분포 줄·느린 예시·시리즈별 통계 표는 여기에만 둔다 — 격자 패널마다 붙이면 높이가 제각각이 된다. 브라우저 뒤로가기가 격자로 돌아오고, `← 전체 메트릭`은 history를 replace해 뒤로가기가 단독 보기를 다시 열지 않는다
...
**업타임 응답 시간 (`ResponseTimeCard`, 2026-09-26 — Datadog Synthetics형)** — 모니터 상세와 Docker 서비스 업타임 탭이 같은 카드를 쓴다. 선 + 실패 음영은 두 번 바꿨다: 전체 높이 음영은 *"실패한 게 너무 우뚝 보인다"*, 바닥 띠는 *"디자인이 아쉽다"*.
- **체크 1회 = 결과 칸 1개 + 막대 1개**, 세로로 맞춘다. 결과 칸은 성공 `status-healthy/45`(뒤로 물러나게) · 일부 실패 `status-warn`(버킷만) · 실패 `status-error`. 막대는 응답 시간 `primary/85`
- **실패는 빨간 결과 칸 + 바닥의 6px 빨간 막대**뿐이다. 실패한 체크의 시간(타임아웃)은 막대 높이가 되지 않는다 — 스케일은 성공한 체크와 알림 임계값만으로 잡는다
- **막대는 시간 위치에 놓인다** (슬롯 폭 = 체크 간격의 중앙값, 막대는 그중 62%). 수집이 끊긴 구간은 빈칸으로 보인다. 버킷 데이터는 조회한 창이 곧 도메인이다
